...
//                }
    ...
    public function parseRefLine($string, $paramKey="")
    {
        ...
        } elseif (strpos($string, 'model\\') !== false) {
            // 引用模型
            $value = $this->parseModel($string);
        } elseif (strpos($string, 'definitions\\') !== false) {
            // 引用定义
    ...
    public function parseModel($path)
    {
        ...
        } elseif (!empty($action_doc) && !empty($action_doc['withoutField'])) {
    ...
    public function filterModelTableField($params, $keys, $type="field")
    {
        ...
        foreach ($params as $modelParam) {
            ...
            } elseif (!(!empty($modelParam['name']) && in_array($modelParam['name'], $fieldArr)) && $type =="withoutField") {
                // 排除指定字段
                $modelParams[]=$modelParam;
            }
        }
